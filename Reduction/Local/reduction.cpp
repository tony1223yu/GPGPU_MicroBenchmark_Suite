...
int main(int argc, char *argv[])
{
    ...
    for (currInputSize = g_opencl_ctrl.padSize ; currInputSize > 1 ; )
    {
        ...
        {
            case TYPE_INT:
                local_mem_size = sizeof(int) * g_opencl_ctrl.local_size;
                ...
                break;
            case TYPE_FLOAT:
                local_mem_size = sizeof(float) * g_opencl_ctrl.local_size;
                ...
                break;
            case TYPE_DOUBLE:
                local_mem_size = sizeof(double) * g_opencl_ctrl.local_size;
        ...
        CHECK_CL_ERROR(error);
        error = clSetKernelArg(kernel, 3, local_mem_size, NULL);
        CHECK_CL_ERROR(error);
