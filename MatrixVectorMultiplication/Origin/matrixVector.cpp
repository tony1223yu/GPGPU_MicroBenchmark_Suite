...
/* Macros */
#define CL_FILE_NAME "matrixVector.cl"
#define BINARY_FILE_NAME "matrixVector.bin"
#define CL_KERNEL "dia_spmv"
...
struct OpenCL_Ctrl
{
    ...
    int device_id;
    bool timing;
    int dataSizeW, dataSizeH, inputByteA, inputByteB, inputByteC, outputByte;
    bool verify;
    char powerFile[POWER_LOG_FILE_LEN];

    OpenCL_Ctrl() : platform_id(0), device_id(0), timing(false), dataSizeW(1024), dataSizeH(1024), verify(false) {sprintf(powerFile, "KernelExecution.log");}
...
void CommandParser(int argc, char *argv[])
{
    ...
    g_opencl_ctrl.inputByteA = g_opencl_ctrl.dataSizeH * g_opencl_ctrl.dataSizeW * sizeof(float); // A
    g_opencl_ctrl.inputByteB = g_opencl_ctrl.dataSizeH * sizeof(float); // offsets
    g_opencl_ctrl.inputByteC = g_opencl_ctrl.dataSizeW * sizeof(float); // x
    g_opencl_ctrl.outputByte = g_opencl_ctrl.dataSizeW * sizeof(float); // y
    
    fprintf(stderr, "inputByte = %d %d %d\n", g_opencl_ctrl.inputByteA, g_opencl_ctrl.inputByteB, g_opencl_ctrl.inputByteC);
...
void HostDataCreation(void* &inputA, void* &inputB, void* &inputC, void* &output)
{
    srand(time(NULL));
    inputA = malloc(g_opencl_ctrl.inputByteA);
    inputB = malloc(g_opencl_ctrl.inputByteB);
    inputC = malloc(g_opencl_ctrl.inputByteC);
    ...
    
    {
        ...
        for (int i = 0 ; i < g_opencl_ctrl.dataSizeH * g_opencl_ctrl.dataSizeW ; i ++)
            tmpA[i] = ((float)rand() / RAND_MAX);
        for (int i = 0 ; i < g_opencl_ctrl.dataSizeH ; i ++)
            tmpB[i] = 0;
        for (int i = 0 ; i < g_opencl_ctrl.dataSizeW ; i ++)
            tmpC[i] = ((float)rand() / RAND_MAX);
/*
        for (int i = 0 ; i < g_opencl_ctrl.dataSizeH * g_opencl_ctrl.dataSizeW ; i ++)
            printf("%f ", tmpA[i]);
        printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    cl_program program;
    cl_kernel kernel;
    cl_mem inputBufferA, inputBufferB, inputBufferC, outputBuffer;
    cl_int error;
    ...
    HostDataCreation(inputMatrixA, inputMatrixB, inputMatrixC, outputMatrix);
    ...
    /* Create kernels */
    kernel = clCreateKernel(program, CL_KERNEL, &error);
    CHECK_CL_ERROR(error);

    /* Create buffers */
    inputBufferA = clCreateBuffer(context, CL_MEM_READ_WRITE, g_opencl_ctrl.inputByteA, NULL, &error);
    CHECK_CL_ERROR(error);
    inputBufferB = clCreateBuffer(context, CL_MEM_READ_WRITE, g_opencl_ctrl.inputByteB, NULL, &error);
    CHECK_CL_ERROR(error);
    inputBufferC = clCreateBuffer(context, CL_MEM_READ_WRITE, g_opencl_ctrl.inputByteC, NULL, &error);
    CHECK_CL_ERROR(error);
    outputBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE, g_opencl_ctrl.outputByte, NULL, &error);
    CHECK_CL_ERROR(error);

    clEnqueueWriteBuffer(command_queue, inputBufferA, CL_TRUE, 0, g_opencl_ctrl.inputByteA, inputMatrixA, 0, NULL, NULL);
    CHECK_CL_ERROR(error);
    clEnqueueWriteBuffer(command_queue, inputBufferB, CL_TRUE, 0, g_opencl_ctrl.inputByteB, inputMatrixB, 0, NULL, NULL);
    CHECK_CL_ERROR(error);
    clEnqueueWriteBuffer(command_queue, inputBufferC, CL_TRUE, 0, g_opencl_ctrl.inputByteC, inputMatrixC, 0, NULL, NULL);
    CHECK_CL_ERROR(error);
    ...
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 1, sizeof(int), &g_opencl_ctrl.dataSizeW);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 2, sizeof(int), &g_opencl_ctrl.dataSizeH);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 3, sizeof(cl_mem), &inputBufferB);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 4, sizeof(cl_mem), &inputBufferC);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 5, sizeof(cl_mem), &outputBuffer);
    CHECK_CL_ERROR(error);
    ...
    error = clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, globalSize, NULL, 0, NULL, NULL);
    CHECK_CL_ERROR(error);
    error = clFinish(command_queue);
    CHECK_CL_ERROR(error);
