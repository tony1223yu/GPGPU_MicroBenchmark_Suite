...
#define CL_KERNEL_BASE "MatrixMultiplication"
...
struct OpenCL_Ctrl
{
    ...
    OpenCL_Ctrl() : kernelNum(1), platform_id(0), device_id(0), timing(false), dataType(TYPE_INT), dataSizeW(1024), dataSizeH(1024), dataSizeM(1024), verify(false), localSize(1) {sprintf(powerFile, "KernelExecution.log");}
...
void CommandParser(int argc, char *argv[])
{
    ...
    while(1)
    {
        cmd = getopt(argc, argv, "P:D:Tt:W:H:VM:O:L:");
...
int main(int argc, char *argv[])
{
    ...
    switch(g_opencl_ctrl.dataType)
    {
        case TYPE_INT:
            char kernelName[200];
            sprintf(kernelName, "%s_%d", CL_KERNEL_BASE, g_opencl_ctrl.kernelNum);
    ...
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 5, g_opencl_ctrl.localSize * g_opencl_ctrl.localSize * sizeof(int), NULL);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 6, g_opencl_ctrl.localSize * g_opencl_ctrl.localSize * sizeof(int), NULL);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel, 7, sizeof(int), &g_opencl_ctrl.localSize);
    CHECK_CL_ERROR(error);
            ...
            case TYPE_INT:
                {
                    ...
                    for (int i = 0 ; i < g_opencl_ctrl.dataSizeH * g_opencl_ctrl.dataSizeW ; i ++)
                        ...
                        {
                            fprintf(stderr, "result mismatch! (%d, %d)\n", cpuOutputMatrix[i], tmpO[i]);
                            check = false;
                            break;
                        }
