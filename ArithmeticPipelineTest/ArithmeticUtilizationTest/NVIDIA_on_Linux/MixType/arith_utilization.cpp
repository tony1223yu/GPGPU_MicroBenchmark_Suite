...
struct OpenCL_Ctrl
{
    ...
    OpenCL_Ctrl() : platform_id(0), device_id(0), global_size(1024), local_size(32), iteration(1000), kernelName(NULL) {sprintf(powerFile, "KernelExecution.log");}
    ~OpenCL_Ctrl()
    {
        if (kernelName)
            free(kernelName);
...
void CreateAndBuildProgram(cl_program &target_program, cl_context context, cl_device_id device, char *fileName)
{
    ...
    target_program = clCreateProgramWithBinary(context, 1, &device, &programSize, (const unsigned char **)(&programSource), &binaryError, &error);
    CHECK_CL_ERROR(error);
