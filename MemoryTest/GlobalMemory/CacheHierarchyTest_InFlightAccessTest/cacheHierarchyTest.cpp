...
#define KERNEL_NAME_MAX_LEN 100
...
struct OpenCL_Ctrl
{
    ...
    int localSize;
    int kernelNum;
    char kernel1[KERNEL_NAME_MAX_LEN];
    char kernel2[KERNEL_NAME_MAX_LEN];
    char powerFile[POWER_LOG_FILE_LEN];

    OpenCL_Ctrl() : platform_id(0), device_id(0), size(1), stride(1), iteration(1), globalSize(1), localSize(1), offset(1), interval(1), kernelNum(1) {sprintf(powerFile, "KernelExecution.log");} 
    ~OpenCL_Ctrl() {}
...
{
    char* short_options = strdup("p:d:s:n:i:o:g:l:i:v:k:");
    ...
    sprintf(g_opencl_ctrl.kernel2, "%s_%d", KERNEL_2, g_opencl_ctrl.kernelNum);
...
int main(int argc, char *argv[])
{
    ...
    /* Create kernels */
    kernel1 = clCreateKernel(program, g_opencl_ctrl.kernel1, &error);
    CHECK_CL_ERROR(error);
    kernel2 = clCreateKernel(program, g_opencl_ctrl.kernel2, &error);
    CHECK_CL_ERROR(error);
    ...
    CHECK_CL_ERROR(error);
    fprintf(stderr, "\n['%s' execution time] %llu ns\n", g_opencl_ctrl.kernel2, (end - start) * 1000);
    fprintf(stdout, "%llu\n", (end - start) * 1000);
