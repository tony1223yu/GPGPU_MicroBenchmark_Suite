...
struct OpenCL_Ctrl
{
    ...
    OpenCL_Ctrl() : launcherSize(32), platform_id(0), device_id(0), size(1), stride(1), iteration(1), globalSize(1), localSize(1), offset(1), interval(0) {sprintf(powerFile, "KernelExecution.log");} 
    ~OpenCL_Ctrl() {}
...
{
    char* short_options = strdup("p:d:s:n:i:o:g:l:v:x:");
    struct option long_options[] =
    {
        ...
        {"launcherSize", required_argument, NULL, 'x'},
        /* option end */
        {0, 0, 0, 0}
    };
    int cmd;
    ...
    while(1)
    {
        ...
        {
            case 'x':
                g_opencl_ctrl.launcherSize = atoi(optarg);
                break;
...
void CreateAndBuildProgram(cl_program &target_program, cl_context context, cl_device_id device, char *fileName)
{
    ...
    sprintf(option, "-cl-opt-disable -D DISPATCH_SIZE=%d", g_opencl_ctrl.launcherSize);
...
{
    srand(time(NULL));
    data = malloc(g_opencl_ctrl.globalSize * sizeof(cl_ulong));
}
...
int main(int argc, char *argv[])
{
    ...
    /* Create buffers */
    buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, g_opencl_ctrl.globalSize * sizeof(cl_ulong), hostData, &error);
    CHECK_CL_ERROR(error);
    ...
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel1, 3, sizeof(int), &g_opencl_ctrl.stride);
    CHECK_CL_ERROR(error);
    error = clSetKernelArg(kernel1, 4, sizeof(int), &g_opencl_ctrl.size);
    CHECK_CL_ERROR(error);
    ...
    error = clEnqueueReadBuffer(command_queue, buffer, CL_TRUE, 0, g_opencl_ctrl.globalSize * sizeof(cl_ulong), hostData, 0, NULL, NULL);
    CHECK_CL_ERROR(error);
