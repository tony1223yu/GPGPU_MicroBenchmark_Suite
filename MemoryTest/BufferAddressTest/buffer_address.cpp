...
void CommandParser(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "executable -k <kernelName> [...]\n");
        exit(1);
    }

    g_opencl_ctrl.dataByte = sizeof(cl_ulong) * 2;
...
int main(int argc, char *argv[])
{
    ...
    /* Create buffers */
    buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, g_opencl_ctrl.dataByte, NULL, &error);
    CHECK_CL_ERROR(error);
    ...
    printf("In kernel location %p %p\n", *(cl_ulong *)(hostData), *((cl_ulong*)(hostData)+1));
