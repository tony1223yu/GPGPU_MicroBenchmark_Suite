...
{	
	...
	for(int m=0 ; m<Width/TILE_W ; ++m)
	{
		...
		{
			Pvalue_1 += Mds[ty][k] * Nds_1[k][tx];
			Pvalue_2 += Mds[ty][k] * Nds_2[k][tx];
		}
...
{	
	cout << "----------------------------------------------Start" << endl;
        cout << "This is Hierarchically Tiling version" << endl;
        cout << "---------------------------------------------------" << endl;
        cout << "Grid  Dimension : " << (Width/TILE_W)/2 << "x" << Width/TILE_W << endl;
	...
	*/
	...
	cudaEventRecord(end, 0);
        ...
        dim3 dimGrid((Width/TILE_W)/2, Width/TILE_W);
        dim3 dimBlock(TILE_W, TILE_W);
        size_t SharedMemoryBytes = sizeof(int)*TILE_W*TILE_W*3;
